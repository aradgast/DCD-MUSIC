...
class MUSIC_2D(MUSIC):
    ...
    def __init__(self, system_model: SystemModel):
        ...
        self._angels = np.linspace(-1 * np.pi / 2 + 10**-6, np.pi / 2 - 10**-6, 180)
        # Assign the frequency for steering vector calculation (multiplied in self.dist to get dist = 1/2)
        f = self.system_model.max_freq[self.system_model.params.signal_type]
    ...
    def get_predictions(self, spectrum: np.ndarray, number_of_sources: int,  is_soft: bool = False) -> tuple:
        ...
        """
        if is_soft:
            return self.maskpeak(spectrum, number_of_sources)
        else:
            # Flatten the spectrum
            spectrum_flatten = spectrum.flatten()
            # Find spectrum peaks
            peaks = list(scipy.signal.find_peaks(spectrum_flatten)[0])
            # Sort the peak by their amplitude
    ...
    def maskpeak(self, spectrum: np.ndarray, number_of_sources: int):
        """
        ...
        """
        top_indxs = np.argpartition(spectrum.reshape(1, -1).squeeze(), -number_of_sources)[-number_of_sources:]
    ...
    def narrowband(
            self, X: np.ndarray, known_num_of_sources: bool = True, mode: str = "sample", model: SubspaceNet = None
    ) -> tuple:
        """
        ...
        if known_num_of_sources:
            ...
            raise Exception("Currently support only known number of sources")
        # Calculate covariance matrix
        covariance_mat = self.calculate_covariance(X=X, mode=mode, model=model)
        # Get noise subspace
        Un, _ = self.subspace_separation(covariance_mat=covariance_mat, M=M)
